...
SELECT id, nombre, descripcion FROM categoria WHERE eliminado = FALSE;
...
SELECT dp.pedido_id, pr.nombre AS producto, dp.cantidad, dp.precio_unitario, dp.subtotal
...
BEGIN
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        ...
        IF v_stock < v_cantidad THEN
            RAISE EXCEPTION 'Stock insuficiente (producto %): hay %, se piden %', v_producto_id, v_stock, v_cantidad;
        END IF;
